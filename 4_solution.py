...
rules = {
  'byr': (re.compile(r'^[0-9]{4}$'), lambda val: 1920 <= int(val) <= 2002),
  'iyr': (re.compile(r'^[0-9]{4}$'), lambda val: 2010 <= int(val) <= 2020),
  'eyr': (re.compile(r'^[0-9]{4}$'), lambda val: 2020 <= int(val) <= 2030),
  ...
  'pid': (re.compile(r'^[0-9]{9}$'), lambda val: True)
}
...
if __name__ == '__main__':
  ...
  passports = [p for p in passports if 0 == len(set(rules.keys()).difference(p.keys()))]
